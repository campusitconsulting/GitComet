...
The corresponding context-menu actions, endpoint-picker headings and selected
picker rows, and history rows use the same compact blue `A` and orange `B`
...
generic extra-small ref chips. Its translucent badge tokens are flattened onto
the popup's raised surface before canvas painting: GPUI's low-level quad painter
does not perform the same alpha composition as a layout element, and passing
the token through directly produces blue-on-blue badges in the light theme.
